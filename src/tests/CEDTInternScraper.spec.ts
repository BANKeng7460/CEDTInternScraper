...
import { getCurrentDateTimeForFilename } from '../functions/getCurrentDateForFileName';
...
test('CEDTInternScraper', async ({ page, loginPage }) => {
  ...
  for (let i = 0; i < positionCount; i++) {
    ...
    const url = page.url();
    const hyperlink = `=HYPERLINK("${url}","ดูรายละเอียด")`
    ...
    let competitionRatio: number | null = null;

    if (seats !== null && Number(seats) > 0) {
      competitionRatio = selections / Number(seats);
    }

    console.log(jobName, companyName, seats, "ที่นั่ง", selections, "คนเลือกตำแหน่งนี้", Compensation, Address,competitionRatio, url)
    results.push({ "jobPosition": jobName, "CompanyName": companyName, seats, selections, "Compensation": Compensation, competitionRatio,url:hyperlink,Address });
  }
  ...
  // เขียนไฟล์ CSV
  fs.writeFileSync(`CEDTInternList${getCurrentDateTimeForFilename()}.csv`, csvData);
